...
[![Latest Release](https://img.shields.io/github/v/release/Xsxdot/go-deploy?style=flat-square)](https://github.com/Xsxdot/go-deploy/releases)
...
**方式一：从 [Releases](https://github.com/Xsxdot/go-deploy/releases) 下载（推荐）**

无需 Go 环境，开箱即用。选择对应系统的预编译包下载，解压后即可运行。包内含 `deploy` 可执行文件、README 及 `samples/` 示例。

| 系统 | 下载文件 | 解压后 |
|------|----------|--------|
| Linux x64 | `deploy-<ver>-linux-amd64.tar.gz` | `./deploy` |
| Linux Arm64 | `deploy-<ver>-linux-arm64.tar.gz` | `./deploy` |
| macOS Intel | `deploy-<ver>-darwin-amd64.tar.gz` | `./deploy` |
| macOS M1/M2/M3 | `deploy-<ver>-darwin-arm64.tar.gz` | `./deploy` |
| Windows x64 | `deploy-<ver>-windows-amd64.zip` | `deploy.exe` |
| Windows Arm64 | `deploy-<ver>-windows-arm64.zip` | `deploy.exe` |

*`<ver>` 替换为实际版本号，如 `v1.0.0`，见 [Releases](https://github.com/Xsxdot/go-deploy/releases) 页面。*

<details>
<summary>Linux/macOS 一键安装（curl）</summary>

**指定版本（将 v1.0.0 替换为 [Releases](https://github.com/Xsxdot/go-deploy/releases) 页版本号）：**
```bash
V="v1.0.0"
curl -sL "https://github.com/Xsxdot/go-deploy/releases/download/${V}/deploy-${V}-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/').tar.gz" | tar -xz
cd deploy-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')
...
**自动获取最新版本：**
```bash
V=$(curl -s https://api.github.com/repos/Xsxdot/go-deploy/releases/latest | grep '"tag_name"' | sed -E 's/.*"([^"]+)".*/\1/')
curl -sL "https://github.com/Xsxdot/go-deploy/releases/download/${V}/deploy-${V}-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/').tar.gz" | tar -xz
cd deploy-$(uname -s | tr '[:upper:]' '[:lower:]')-$(uname -m | sed 's/x86_64/amd64/;s/aarch64/arm64/')
sudo mv deploy /usr/local/bin/
deploy --version
...
1. 在 [Releases](https://github.com/Xsxdot/go-deploy/releases) 下载 `deploy-<ver>-windows-amd64.zip`  
2. 解压后将目录加入 PATH，或在解压目录打开终端运行 `.\deploy.exe`
</details>
